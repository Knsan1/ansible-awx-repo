...
def main():
    ...
    try:
        ...
        secret_id = secret_data.get(args.key, {}).get(args.subkey)
        if not secret_id:
            print(f"❌ Failed to retrieve '{args.key}.{args.subkey}' from Vault.")
            sys.exit(1)
        print("✅ Retrieved secret_id from Vault")
    except Exception as e:
